...
async function extractPalettes(palettesDiv) {
  ...
  for (const { url, title } of imageFiles) {  // Destructure the url and title
      try {
          ...
          paletteContainer.style.width = '30%'; // Use 30% to allow three columns (adjust as needed)
          paletteContainer.style.margin = '10px'; // Add some spacing around each palette
          paletteContainer.style.display = 'flex'; // Keep the display as flex
          paletteContainer.style.flexDirection = 'column'; // Align items vertically
