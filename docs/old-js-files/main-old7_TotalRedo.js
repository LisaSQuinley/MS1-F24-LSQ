...
async function extractPalettes(palettesDiv, paletteTooltip) {
  ...
  const groupedPalettes = []; // Change to an array instead of an object
  ...
  for (const { url, title } of imageFiles) {
    try {
      ...
      for (const key of paletteKeys) {
        ...
        if (swatch) {
          // Directly push swatch data into the array
  ...
  // You can continue with your logic using groupedPalettes without categories
  const concatArray = groupedPalettes; // No need to flatten
